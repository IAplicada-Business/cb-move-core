-- RPCs para conta vinculada e logs de uso do fisio (gestão/recepção — bypass RLS profiles)

CREATE OR REPLACE FUNCTION public.get_fisio_conta_vinculada(p_fisio_id uuid)
...
BEGIN
  ...
  WHERE p.fisioterapeuta_id = p_fisio_id
  ORDER BY p.created_at
  LIMIT 1;
END;
$$;

GRANT EXECUTE ON FUNCTION public.get_fisio_conta_vinculada(uuid) TO authenticated;
...
GRANT EXECUTE ON FUNCTION public.get_fisio_uso_logs(uuid, int) TO authenticated;

-- Corrigir cobrança Samir: depósito paciente não deve ser tipo convênio
UPDATE public.cobrancas c
SET tipo = p.tipo
FROM public.pacientes p
WHERE c.paciente_id = p.id
  AND c.paciente_id = 'ee171e6a-538e-48a0-bb56-b284c55c36c3'::uuid
  AND c.observacoes ILIKE '%Depósito paciente jun/2026%'
  AND c.tipo IS DISTINCT FROM p.tipo;
