...
import { queueBoletoCobranca } from "../_shared/boleto-cobranca-queue.ts";
...
serve(async (req) => {
  ...
  try {
    ...
    const { cobranca_id, event_id, reenvio } = body;
    ...
    const result = await queueBoletoCobranca(admin, cobranca_id, {
      eventId: event_id,
