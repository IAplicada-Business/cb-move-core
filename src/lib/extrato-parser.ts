...
export function matchTransacoesComCobrancas(
  ...
  const pendentes = cobrancas.filter(c =>
    ['pendente', 'vencido', 'atrasado', 'aguardando_convenio', 'aguardando_alvara'].includes(c.status)
  );
  ...
  for (const t of transacoes.filter(t => t.tipo === 'credito')) {
    ...
    for (const c of porValor) {
      const dias = Math.abs(new Date(t.data).getTime() - new Date(c.vencimento).getTime()) / (1000 * 60 * 60 * 24);
      if (dias < menorDias) {
        menorDias = dias;
        melhor = c;
      }
