...
function montarResumoCronologico(
  ...
  "itens" | "extras" | "faltantes" | "agendadasNoPlano" | "concluidas" | "pendentes" | "faltas" | "faltantesSlots"
> {
...
> {
  ...
  const { slots: casados, faltantesSlots, extras: extrasAg } = mapearAgendamentosAosSlots(
    slots,
    input.agendamentos,
  );
...
export function montarResumoPlanoSessoesMensal(input: {
  ...
  const usarSlots =
    diasPt.length > 0 && quantidadeMensal != null && quantidadeMensal > 0;
